...
def bgp_filter(dev):
    ...
    var_data = {}
    for data in rpc_xml.findall('bgp-peer'):
        state_type = data.find('peer-type').text
        group_name = data.find('peer-group').text
        if state_type == 'External':
            for policy in data.findall('bgp-option-information'):
                if policy.find('import-policy') is not None:
                    # print(policy.find('import-policy').text.strip(), '--->', group_name)
                    policy_name = policy.find('import-policy').text.strip()
                    var_data[group_name] = policy_name
# print(var_data)   
    conf_file = Template(open('Insert-command.j2').read())
